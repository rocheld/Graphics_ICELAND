...
void RasterizerQuad::rasterize(glm::mat4 M, glm::mat4 V, glm::mat4 P, glm::mat4 D, PointCloud& obj,int pointSize) {
	/*
    ...
    float z;
    PointCloud* curr = &obj;
    std::vector<glm::vec3> points = curr->getPoints();
    std::vector<glm::vec3> normal = curr->getNormals();
    
    for(int i=0; i < points.size(); i++) {
        glm::vec3 curr = points[i];
        glm::vec4 a = vec4(curr,1.0f);
        glm::vec3 norm = normal[i];
        glm::vec4 aa = D*P*V*M*a;
        
        z = aa[2]/aa[3];
        x = aa[0]/aa[3];
        y = aa[1]/aa[3];
        // if z is too close to the screen, don't draw.
        if(z > 1.0f) {
            continue;
        }
        drawPoint(x,y,z,norm[0],norm[1],norm[2]);
        
        if(pointSize > 1) {
            for(int j=0; j<pointSize; j++) {
                for(int k=0; k < pointSize;k++) {
                    drawPoint(x + j,y+ k,z,norm[0],norm[1],norm[2]);
                }
            }
        }
